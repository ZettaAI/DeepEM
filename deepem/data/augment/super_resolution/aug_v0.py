...
- Isotropic data:  iso-safe augments (3D) at full iso resolution.
  Input degradation (avg-downsample + zero-pad) is handled by the sampler.
...
def get_augmentation(
    ...
    **kwargs,
    ...
    Input degradation (avg-downsample + zero-pad) is handled by the sampler,
    not the augmentation pipeline.

    Args:
        is_train: Whether this is for training (enables more augmentations)
        sr_mode: Enable super-resolution mode (unused, kept for API compat)
        sr_scale_z: Z downsampling factor (unused, kept for API compat)
