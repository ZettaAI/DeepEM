...
def get_augmentation(is_train, box=None, missing=7, blur=7, lost=True,
    ...
    # (1) Misalingment
    trans = Compose([Misalign((0,4), margin=1),
                     Misalign((0,12), margin=1),
                     Misalign((0,20), margin=1)])
    slip = Compose([SlipMisalign((0,4), interp=True, margin=1),
                    SlipMisalign((0,12), interp=True, margin=1),
                    SlipMisalign((0,20), interp=True, margin=1)])
    mutex.append(Blend([trans,slip], props=[0.7,0.3]))

    # (2) Misalignment + missing section
    if is_train:
        mutex.append(Blend([
            MisalignPlusMissing((3,10), value=0, random=random),
            MisalignPlusMissing((3,10), value=0, random=False)
        ]))
    else:
        mutex.append(MisalignPlusMissing((3,10), value=0, random=False))
    ...
    if is_train:
        if box == 'noise':
            augs.append(
                NoiseBox(sigma=(1,3), dims=(3,15), margin=(1,5,5),
                         density=0.3, skip=0.1)
            )
        elif box == 'fill':
            augs.append(
                FillBox(dims=(3,15), margin=(1,5,5),
                        density=0.3, skip=0.1)
            )
