...
def get_augmentation(is_train, box=None, missing=7, blur=7, lost=True,
    ...
    # (1) Misalingment
    trans = Compose([Misalign((0, 2), margin=1),
                     Misalign((0,4), margin=1),
                     Misalign((0,6), margin=1)])
    slip = Compose([SlipMisalign((0, 2), interp=True, margin=1),
                    SlipMisalign((0,4), interp=True, margin=1),
                    SlipMisalign((0,6), interp=True, margin=1)])
    ...
    if is_train:
        if box == 'noise':
            augs.append(
                NoiseBox(sigma=(1,3), dims=(1,6), margin=(1,5,5),
                         density=0.3, skip=0.1)
            )
        elif box == 'fill':
            augs.append(
                FillBox(dims=(1,6), margin=(1,5,5),
                        density=0.3, skip=0.1)
            )
